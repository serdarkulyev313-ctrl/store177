...
import { validateProductOptions } from "@/lib/productOptionsStore";
import { getProductOptions, saveProductOptions, ProductOptionsPayload } from "@/lib/db/options";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || "Ошибка чтения опций" },
...
async function handleWrite(req: Request) {
  ...
  const body = (await req.json()) as { options?: ProductOptionsPayload };
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || "Ошибка сохранения опций" },
      { status: 500, headers: { "Cache-Control": "no-store" } }
    );
  }
}
